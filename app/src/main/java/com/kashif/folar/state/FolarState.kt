 ...
 * @example
 * ```kotlin
 * val cameraState by rememberFolarState(configuration)
 * when (cameraState) {
 *     is FolarState.Loading -> LoadingIndicator()
 *     is FolarState.Ready -> CameraPreviewUI(cameraState.controller)
 *     is FolarState.Error -> ErrorScreen(cameraState.exception)
 * }
 * ```
 */
@Immutable
sealed class FolarState {
    /**
     * Camera is initializing. No controller available yet.
     * Show loading UI or progress indicator in this state.
     */
    @Immutable
    data object Initializing : FolarState()
 ...
 */
@Immutable
sealed class FolarEvent {
    /**
     * No event (initial state).
     */
    @Immutable
    data object None : FolarEvent()
     ...
     */
    @Immutable
    data class ImageCaptured(val result: ImageCaptureResult) : FolarEvent()
     ...
     */
    @Immutable
    data class TextRecognized(val text: String) : FolarEvent()
     ...
     */
    @Immutable
    data class PermissionDenied(val permission: String) : FolarEvent()
}
 ...
 * )
 * val cameraState by rememberFolarState(config)
 * ```
 */
@Immutable
